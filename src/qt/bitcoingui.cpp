...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a newpiggycoin address"));
...
//	connect(poolAction, SIGNAL(triggered()), this, SLOT(gotoPoolBrowser()));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for newpiggycoin"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("newpiggycoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("newpiggycoin client"));
...
{
    QDesktopServices::openUrl(QUrl("http://www.piggy-coin.com/"));
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to newpiggycoin network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid newpiggycoin address or malformed URI parameters."));
    }
...
void BitcoinGUI::backupWallet()
{
    ...
    QString filename = QFileDialog::getSaveFileName(this, tr("Backup Piggybank"), saveDir, tr("Piggybank Data (*.dat)"));
    if(!filename.isEmpty()) {
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    else
    {
        ...
        if (pwalletMain && pwalletMain->IsLocked())
            labelStakingIcon->setToolTip(tr("Not staking because piggybank is locked"));
        else if (vNodes.empty())
            labelStakingIcon->setToolTip(tr("Not staking because piggybank is offline"));
        else if (IsInitialBlockDownload())
            labelStakingIcon->setToolTip(tr("Not staking because piggybank is syncing"));
        else if (!nWeight)
